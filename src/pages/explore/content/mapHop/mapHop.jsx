...
import '../globaleMap/globaleMap.scss';
import './mapHop.scss'
...
import data from '../globaleMap/data.json';
...
function GlobaleMap() {
  ...
  const { category } = useParams(); // Récupère le paramètre d'URL
  ...
  useEffect(() => {
    ...
    const filtered = data.categories.filter((cat) => cat.nom.toUpperCase() === category.toUpperCase());
                    ...
                    {category.lieux
                      ...
                      .map((place, index) => (
                        <div key={index} className="place-card">
                          <h3>{place.nom}</h3>
                          <p>{place.description}</p>
                          {/* Ajoute ici des boutons selon la catégorie */}
                          <button onClick={() => redirectToVisit(index)}>
                            Visiter le lieu
                          </button>
                        </div>
                      ))}
